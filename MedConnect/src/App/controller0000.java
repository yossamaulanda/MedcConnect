...
public class controller0000 implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        userList.loadUser(); // Pastikan data user dimuat saat controller diinisialisasi
    ...
    private void handleLoginButton(ActionEvent event) {
        ...
        if (loggedInUser != null) {
            ...
            if ("Dokter".equals(loggedInUser.getRole())) {
                directToHalamanDokter();
            } else if ("Pasien".equals(loggedInUser.getRole())) {
                directToHalamanDokter();
            } else {
                // Handle other roles or default to halamandokter
                directToHalamanDokter();
            }
        } else {
            // Login failed - add error styling
            addErrorStyling();
    ...
    private void directToRegistrationPage() {
        try {
            OpenScene openScene = new OpenScene();
            Pane halaman = openScene.getPane("halaman00000.fxml");
    ...
    private void directToHalamanDokter() {
        try {
            OpenScene openScene = new OpenScene();
            Pane halaman = openScene.getPane("halamandokter.fxml");
            
            // Add transition animation
            addPageTransition(halaman);
            
            mainPane.setCenter(halaman);
            System.out.println("Redirected to halamandokter");
        } catch (Exception e) {
            showStyledAlert(AlertType.ERROR, "Navigation Error", "Could not load halamandokter: " + e.getMessage());
        }
    }
